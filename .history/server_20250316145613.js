...
app.post("/actualizar-stock", (req, res) => {
  ...
  app.post("/finalizar-compra", (req, res) => {
    const { us_id, productos, total } = req.body;
  
    if (!us_id || !productos || productos.length === 0 || !total) {
    ...
    db.beginTransaction((err) => {
      ...
      // Insertar en freshboxordenes
      const ordenQuery = "INSERT INTO freshboxordenes (us_id, orden_total, orden_fecha) VALUES (?, ?, ?)";
      db.query(ordenQuery, [us_id, total, fechaActual], (err, result) => {
        if (err) {
          return db.rollback(() => {
        ...
        const detallesQuery = "INSERT INTO freshboxdetallesordenes (orden_id, pr_id, cantidad, precio_unitario) VALUES ?";
        const detallesValores = productos.map(p => [orden_id, p.pr_id, p.cantidad, p.precio]);
  
        db.query(detallesQuery, [detallesValores], (err) => {
          if (err) {
            return db.rollback(() => {
              console.error("Error al insertar detalles de orden:", err);
              res.status(500).json({ success: false, message: "Error al registrar detalles de orden" });
            });
          }
          ...
          const updateStockQueries = productos.map(p => {
            return new Promise((resolve, reject) => {
              db.query(
                "UPDATE freshboxproductos SET pr_cantidad = pr_cantidad - ? WHERE pr_id = ? AND pr_cantidad >= ?",
                [p.cantidad, p.pr_id, p.cantidad],
                (err, result) => {
                  if (err || result.affectedRows === 0) {
                    return reject(new Error(`Stock insuficiente para producto ID ${p.pr_id}`));
                  }
                  resolve();
                }
              );
          ...
          Promise.all(updateStockQueries)
            .then(() => {
              db.commit((err) => {
                ...
                res.json({ success: true, message: "Compra finalizada correctamente", orden_id });
              });
            })
            .catch((error) => {
              db.rollback(() => {
