...
app.put("/editar-producto/:id", (req, res) => {
  ...
  db.query(
      "UPDATE freshboxproductos SET pr_nombre = ?, pr_precio = ?, pr_cantidad = ?, pr_descripcion = ?, pr_imagen = ?, pr_temperatura = ?, pr_status = ?, pr_peso = ? WHERE pr_id = ?",
      [pr_nombre, pr_precio, pr_cantidad, pr_descripcion, pr_imagen, pr_temperatura, pr_status, pr_peso, id],
      (err, result) => {
          if (err) {
