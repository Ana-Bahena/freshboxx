  ...
  app.post("/insert-productos", (req, res) => {
    ...
    const query = "INSERT INTO freshboxproductos (pr_nombre, pr_precio, pr_cantidad, pr_descripcion, pr_imagen, pr_temperatura, pr_status, pr_peso) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    db.query(query, [pr_nombre, pr_precio, pr_cantidad, pr_descripcion, pr_imagen, pr_temperatura, pr_status, pr_peso], (err, result) => {
        if (err) {
            return res.status(500).json({ success: false, message: "Error al insertar el producto" });
        }
        res.json({ success: true, message: "Producto agregado correctamente", producto: { id: result.insertId, ...req.body } });
    });
});
