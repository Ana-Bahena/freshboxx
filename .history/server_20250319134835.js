...
app.post("/agregar-contenedor", (req, res) => {
  ...
  const cantidad = 0;
  const query = "INSERT INTO freshboxcontenedores (ct_cantidad, ct_peso, ct_status) VALUES (?, ?)";
  const [contenedor, setContenedor] = useState({
    peso: "",
    status: "Disponible",
    cantidad: 0  // nuevo valor por defecto
  });
  db.query(query, [cantidad, peso, status], (err, result) => {
    if (err) {
