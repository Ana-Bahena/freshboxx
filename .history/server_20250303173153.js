...
app.post("/login", (req, res) => {
  ...
  db.query("SELECT * FROM freshboxusuarios WHERE us_email = ?", [email], async (err, results) => {
    ...
    // Si el usuario es admin, comparar directamente la contraseña sin bcrypt
    if (user.us_tipo === "admin") {
      if (user.us_password === password) {
        return res.json({ success: true, us_id: user.us_id, us_tipo: user.us_tipo });
      } else {
        return res.status(401).json({ success: false, message: "Contraseña incorrecta" });
      }
    }

    // Para clientes, comparar con bcrypt
