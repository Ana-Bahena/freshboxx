...
db.connect((err) => {
  ...
  app.post("/login", (req, res) => {
    ...
    const sql = "SELECT * FROM freshboxusuarios WHERE us_email = ?";
    ...
    db.query(sql, [email], (err, results) => {
        ...
        if (user.us_password === password) { // Si usas bcrypt, cambia esta validación
            const updateSession = "UPDATE freshboxusuarios SET us_sesion_activa = TRUE WHERE us_email = ?";
            db.query(updateSession, [email], () => {
  ...
  app.post("/logout", (req, res) => {
    if (!req.session.userId) {
      return res.json({ success: false, message: "No hay una sesión activa" });
    }
  
    db.query(
      "UPDATE freshboxusuarios SET us_sesion_activa = 0 WHERE us_id = ?",
      [req.session.userId]
    );
