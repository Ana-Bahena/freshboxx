...
// Aquí defines la función obtenerCliente
...
const obtenerDetallesVenta = async (us_id) => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT producto_nombre, cantidad, precio, (cantidad * precio) AS subtotal
       FROM detalles_venta WHERE us_id = ?`,
      [us_id],
      (err, results) => {
        if (err) {
          reject("Error al obtener detalles de la venta: " + err);
        } else if (results.length === 0) {
          reject("No se encontraron productos para esta venta.");
        } else {
          resolve(results);
        }
      }
...
app.get("/descargar-factura", async (req, res) => {
  ...
  try {
    ...
    detallesVenta.forEach((producto) => {
      ...
      doc.text(`Producto: ${producto_nombre}`, { continued: true });
      doc.text(` | Cantidad: ${cantidad}`, { continued: true });
      doc.text(` | Precio: $${precio.toFixed(2)}`, { continued: true });
