...
app.post("/login", (req, res) => {
  ...
  const sql = "SELECT us_email, us_password FROM freshboxusuarios WHERE us_email = ?";
  ...
  db.query(sql, [email], (err, results) => {
      ...
      // Si la contraseña está almacenada en texto plano (NO RECOMENDADO), usa esta comparación:
      if (password === user.us_password) {
          return res.json({ message: "Inicio de sesión exitoso", user });
      }

      // Si la contraseña está cifrada con bcrypt, usa esta comparación:
      bcrypt.compare(password, user.us_password, (err, isMatch) => {
          if (err) {
