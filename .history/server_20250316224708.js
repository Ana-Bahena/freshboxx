...
app.get("/descargar-factura", async (req, res) => {
  ...
  try {
    ...
    // Crear la tabla de productos con bordes
...
doc.rect(itemX, tableTop, itemWidth, rowHeight).stroke();
doc.rect(qtyX, tableTop, qtyWidth, rowHeight).stroke();
doc.rect(priceX, tableTop, priceWidth, rowHeight).stroke();
doc.rect(subtotalX, tableTop, totalWidth, rowHeight).stroke();

doc.text("Producto", itemX + 5, tableTop + 5, { width: itemWidth - 10, align: "left" });
doc.text("Cantidad", qtyX + 5, tableTop + 5, { width: qtyWidth - 10, align: "center" });
doc.text("Precio", priceX + 5, tableTop + 5, { width: priceWidth - 10, align: "center" });
doc.text("Subtotal", subtotalX + 5, tableTop + 5, { width: totalWidth - 10, align: "center" });

// Dibujar los productos con bordes
...
detallesVenta.forEach((producto) => {
  ...
  // Dibujar rectángulos por cada celda
  ...
  doc.rect(subtotalX, yPosition, totalWidth, rowHeight).stroke();

  // Texto dentro de las celdas
  doc.text(pr_nombre, itemX + 5, yPosition + 5, { width: itemWidth - 10, align: "left" });
  ...
  doc.text(`$${subtotalNum.toFixed(2)}`, subtotalX + 5, yPosition + 5, { width: totalWidth - 10, align: "center" });
