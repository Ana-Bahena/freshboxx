...
const db = mysql.createConnection({
  ...
  database: process.env.DB_DATABASE || "sql5769755",
  //port: process.env.DB_PORT || 3306,
...
app.post("/google", async (req, res) => {
  ...
  try {
    ...
    db.query("SELECT * FROM usuarios WHERE us_email = ?", [email], async (err, results) => {
      ...
      if (results.length > 0) {
        ...
        if (existingUser.us_tipo !== "cliente") {
          return res.status(403).json({ success: false, message: "Acceso denegado. Solo usuarios de tipo cliente pueden ingresar." });
        }

        // Usuario válido, devolver datos
        return res.json({ success: true, user: existingUser });
      } else {
        // Nuevo usuario, registrarlo como "cliente"
        const hashedPassword = await bcrypt.hash(email, 10);
        const newUser = { us_email: email, us_password: hashedPassword, us_tipo: "cliente", us_estatus: "activo" };
