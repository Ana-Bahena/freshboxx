...
const GraciasCompra = () => {
  ...
  const descargarFactura = async () => {
    ...
    try {
      app.get("/test", (req, res) => res.send("Servidor activo"));

      const response = await fetch(`http://localhost:5001/api/descargar-factura?us_id=${userId}`, {
        method: "GET",
      });
  
      console.log("Status:", response.status);  // 👈 Nuevo log
  
      if (!response.ok) {
        const errorText = await response.text();
        console.log("Servidor iniciado...")
        console.error("Texto del error:", errorText);  // 👈 Mostrar el mensaje
