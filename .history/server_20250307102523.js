...
app.post("/usuarios", (req, res) => {
  ...
  db.query(
    "INSERT INTO freshboxusuarios (us_email, us_password, us_tipo, us_estatus, us_nombre, us_telefono, us_direccion, us_rfc, us_placas) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [email, password, tipo, estatus, nombre, telefono, direccion, rfc, placas],
    (err, result) => {
      if (err) {
...
app.post("/register", async (req, res) => {
  const { nombre, email, password, telefono, direccion, rfc, placas, tipo } = req.body;
  ...
  try {
    ...
    // Insertar en la tabla `usuarios`
    db.query(
      "INSERT INTO freshboxusuarios (us_email, us_password, us_tipo, us_estatus) VALUES (?, ?, ?, 'Activo')",
      [email, hashedPassword, tipo],
      (err, result) => {
        if (err) {
          console.error("Error al registrar en usuarios:", err);
          return res.status(500).json({ success: false, message: "Error al registrar en usuarios" });
        }
        const us_id = result.insertId; // Obtener el ID del usuario insertado
        ...
        if (tipo === "transportista") {
          ...
          db.query(
            "INSERT INTO freshboxtransportistas (tr_nombre, tr_email, tr_telefono, tr_direccion, tr_rfc, tr_placas, us_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
            [nombre, email, telefono, direccion, rfc, placas, us_id],
            (err) => {
              if (err) {
                console.error("Error al registrar en transportistas:", err);
                return res.status(500).json({ success: false, message: "Error al registrar en transportistas" });
              }
