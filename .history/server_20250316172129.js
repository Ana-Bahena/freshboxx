...
app.post("/actualizar-stock", (req, res) => {
  ...
  app.post('/api/finalizarcompra', (req, res) => {
    ...
    console.log('Cliente ID:', cl_id); 
  
    if (!carrito || carrito.length === 0 || !cl_id || !vt_idFactura) {
    ...
    db.beginTransaction((err) => {
      ...
      db.query(ventaQuery, [total, fechaActual, 0, cl_id, vt_idFactura], (err, result) => {
        if (err) {
          return db.rollback(() => {
        ...
        const carritoValores = carrito.map(p => [
          ...
          p.precio * p.cantidad, 
          p.precio, 
          cl_id, // Sin transportista asignado
