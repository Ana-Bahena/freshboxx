...
doc.fontSize(12).text("Producto", 50, doc.y, { continued: true });
...
// Iterar sobre los productos y añadir cada fila
detallesVenta.forEach((producto, index) => {
  const { producto_nombre, cantidad, precio, subtotal } = producto;
  total += subtotal;

  // Renderizar los datos del producto en sus respectivas columnas
  doc.fontSize(10).text(producto_nombre, 50, doc.y, { continued: true });
  doc.text(cantidad.toString(), 200, doc.y, { continued: true });
  doc.text(`$${precio.toFixed(2)}`, 300, doc.y, { continued: true });
...
// Mostrar el total
doc.fontSize(12).text(`Total: $${total.toFixed(2)}`, 400, doc.y, { align: "right" });
