...
app.get("/descargar-factura", async (req, res) => {
  ...
  const precioSinIVA = parseFloat(precio);
  const iva = precioSinIVA * 0.16;
  ...
  doc.text(cantidad, qtyX + 5, yPosition + 5, { width: qtyWidth - 10, align: "center" });
  doc.text(`$${precioSinIVA.toFixed(2)}`, priceX + 5, yPosition + 5, { width: priceWidth - 10, align: "center" });
  doc.text(`$${iva.toFixed(2)}`, priceX + 80, yPosition + 5, { width: priceWidth - 10, align: "center" });
  doc.text(`$${precioConIVA.toFixed(2)}`, priceX + 160, yPosition + 5, { width: priceWidth - 10, align: "center" });
  doc.text(`$${subtotal.toFixed(2)}`, subtotalX + 5, yPosition + 5, { width: subtotalWidth - 10, align: "center" });
