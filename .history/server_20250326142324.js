 ...
 //const mysql = require('mysql');
  ...
  const db = mysql.createPool({
    ...
    user: process.env.DB_USERNAME,  // Estás usando DB_USER en el código, pero en el .env es DB_USERNAME
    password: process.env.DB_PASSWORD,
    database: process.env.DB_DATABASE, // También DB_NAME está mal, en el .env es DB_DATABASE
...
const db = mysql.createConnection({
  ...
  port: process.env.DB_PORT || 3306,
  connectTimeout: 70000 // 10 segundos de espera antes de fallar
});
*/
/*
  const db = mysql.createConnection({
    host: process.env.DB_HOST || "dtai.uteq.edu.mx",
    user: process.env.DB_USER || "bahana228",
    password: process.env.DB_PASSWORD || "bsa@2022371113",
    database: process.env.DB_NAME || "bd_awos_bahana228"
  }); 
*//*
  db.connect((err) => {
    if (err) {
  ...
  app.get("/ping", (req, res) => {
    db.query("SELECT 1", (err, results) => {
      if (err) {
        ...
        return res.status(500).json({ success: false, message: "Error de conexión a MySQL" });
      }
