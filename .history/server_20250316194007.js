...
app.post("/actualizar-stock", (req, res) => {
  ...
  const { us_id } = req.query;
  console.log("Solicitud de factura para us_id:", us_id);

  if (!us_id) {
    return res.status(400).json({ success: false, message: "Falta el ID de usuario." });
  }

  // Suponiendo que obtienes los datos del cliente de una base de datos
  const cliente = await obtenerCliente(us_id);  // Función que obtienes desde DB
  ...
  // Aquí puedes incluir más detalles como los datos del cliente, etc.
  doc.fontSize(20).text("Factura", { align: "center" });
  doc.moveDown();
  doc.fontSize(14).text(`Usuario ID: ${us_id}`);
