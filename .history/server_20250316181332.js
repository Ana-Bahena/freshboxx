...
app.post("/actualizar-stock", (req, res) => {
  ...
  app.post('/api/finalizarcompra', (req, res) => {
    ...
    const buscarClienteQuery = "SELECT cl_id FROM clientes WHERE us_id = ?";
db.query(buscarClienteQuery, [us_id], (err, results) => {
  ...
  const cl_id = results[0].cl_id;
    ...
    db.beginTransaction((err) => {
      ...
      const ventaQuery = "INSERT INTO freshboxventas (vt_total, vt_fecha, vt_status, vt_pesoTotal, vt_formaPago, us_id, vt_idFactura) VALUES (?, date, 'pendiente', ?, 'paypal', ?, ?)";
      db.query(ventaQuery, [total, fechaActual, 0, us_id, vt_idFactura], (err, result) => {
        if (err) {
          return db.rollback(() => {
