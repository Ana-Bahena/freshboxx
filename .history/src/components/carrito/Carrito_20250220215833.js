...
function Carrito({ carrito, setCarrito }) {
  ...
  const cambiarCantidad = (id, nuevaCantidad) => {
    ...
    if (nuevaCantidad > cantidadDisponible) {
      // Establecer el mensaje de error solo en el producto específico
      ...
      const nuevoCarrito = carrito.map((producto) => {
        if (producto.pr_id === id) {
          return { ...producto, cantidad: nuevaCantidad, error: "" }; // Limpiar error al cambiar la cantidad
        }
        return producto;
      });
      setCarrito(nuevoCarrito);
  ...
  // Función para finalizar la compra y redirigir a Pago
  const finalizarCompra = () => {
    // Aquí puedes realizar alguna lógica si es necesario antes de redirigir
    navigate("/pago"); // Cambia la ruta según sea necesario
  };
