...
app.post("/actualizar-stock", (req, res) => {
  ...
  app.post('/api/finalizarcompra', (req, res) => {
    ...
    db.beginTransaction((err) => {
      ...
      const ventaQuery = "INSERT INTO freshboxventas (vt_total, vt_fecha, vt_status, vt_pesoTotal, vt_formaPago, cl_id) VALUES (?, date, 'pendiente', ?, 'paypal', ?)";
      db.query(ventaQuery, [total, fechaActual, 0, cl_id], (err, result) => {
        console.log(req.body);
        if (err) {
          return db.rollback(() => {
            console.log(req.body);
