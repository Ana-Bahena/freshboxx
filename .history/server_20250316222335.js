...
const obtenerDetallesVenta = async (us_id) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(
      "SELECT vt_id FROM const obtenerDetallesVenta = async (us_id) => {
  return new Promise((resolve, reject) => {
    // Obtener el ID de la última venta realizada por el usuario
    db.query(
      ...
      (err, results) => {
        if (err) {
          ...
          db.query(
            "SELECT cc.cc_cantidad AS cantidad, cc.cc_precio AS precio, cc.cc_subtotal AS subtotal " +
...
freshboxventas WHERE us_id = ? ORDER BY vt_fecha DESC LIMIT 1",
      [us_id],
      (err, results) => {
        if (err) {
          reject("Error al obtener la venta: " + err);
          ...
          // Obtener los detalles de la venta (productos, cantidad, precio, subtotal)
          db.query(
            "SELECT p.producto_nombre, cc.cc_cantidad AS cantidad, cc.cc_precio AS precio, cc.cc_subtotal AS subtotal " +
            "FROM freshboxcarritocompras cc " +
            "JOIN productos p ON cc.pr_id = p.pr_id " +
            "WHERE cc.vt_id = ?",
            [vt_id],
            (err, results) => {
              if (err) {
