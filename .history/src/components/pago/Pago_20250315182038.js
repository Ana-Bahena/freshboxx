...
const PayPalButtonComponent = () => {
  ...
  const onApprove = (data, actions) => {
    return actions.order.capture().then(async function (details) {
      ...
      const ventaData = {
        cl_id: 1, // ID del cliente
        vt_total: carrito.reduce((total, p) => total + p.precio * p.cantidad, 0), // Total de la venta
        vt_pesoTotal: carrito.reduce((total, p) => total + (p.peso || 0) * p.cantidad, 0), // Peso total
        vt_formaPago: "paypal", // Método de pago
        vt_fecha: new Date().toISOString().slice(0, 19).replace("T", " "), // Fecha de la venta
        carrito: carrito.map(p => ({
          pr_id: p.pr_id, // ID del producto
