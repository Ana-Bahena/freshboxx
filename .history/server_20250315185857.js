...
app.post("/actualizar-stock", (req, res) => {
  ...
  const queries = productos.map((producto) => {
    return new Promise((resolve, reject) => {
      db.query(
        ...
        [producto.cantidad, producto.pr_id], // <-- Cambiado de producto.id a producto.pr_id
        (err, result) => {
          if (err) {
  ...
  app.post("/registrar-venta", (req, res) => {
    console.log("Datos recibidos:", req.body); // Agregar este log para depuración
    const { cl_id, vt_total, vt_pesoTotal, vt_formaPago, vt_fecha, carrito } = req.body;
  
    if (vt_total === null || vt_total === undefined) {
      return res.status(400).json({ success: false, message: "El total de la venta no puede ser null" });
    }
    // Verificar que se haya recibido un carrito
    if (!carrito || carrito.length === 0) {
      return res.status(400).json({ success: false, message: "Carrito vacío" });
    }
  
    // Iniciar transacción para asegurar que la venta y actualización de stock se realicen correctamente
    db.beginTransaction((err) => {
      if (err) {
        return res.status(500).json({ success: false, message: "Error en la transacción" });
      }
  
      // Insertar la venta en la tabla freshboxventas
      const ventaQuery = `
        INSERT INTO freshboxventas (cl_id, vt_total, vt_pesoTotal, vt_formaPago, vt_fecha, vt_status)
        VALUES (?, ?, ?, ?, ?, 'pendiente')`;
      db.query(ventaQuery, [cl_id, vt_total, vt_pesoTotal, vt_formaPago, vt_fecha], (err, result) => {
        if (err) {
          return db.rollback(() => {
        ...
        const vt_id = result.insertId; // ID de la venta recién insertada
  
        // Insertar los productos en freshboxcarritocompras
        const carritoQueries = carrito.map((producto) => {
          return new Promise((resolve, reject) => {
            const carritoQuery = `
              INSERT INTO freshboxcarritocompras (cc_cantidad, cc_subtotal, cc_precio, ts_id, vt_id, pr_id)
              VALUES (?, ?, ?, ?, ?, ?)`;
            db.query(
              carritoQuery,
              [producto.cantidad, producto.subtotal, producto.precio, producto.ts_id, vt_id, producto.pr_id],
              (err, result) => {
                if (err) {
                  reject(err);
                } else {
        ...
        // Ejecutar todas las consultas de inserción del carrito
            ...
            db.commit((err) => {
              if (err) {
                return db.rollback(() => {
                  ...
                  res.status(500).json({ success: false, message: "Error al registrar la venta y actualizar el carrito" });
                });
              }
              res.json({ success: true, message: "Venta registrada y carrito actualizado correctamente" });
            });
          })
          .catch((error) => {
            // Si alguna de las consultas falla, hacer rollback
            db.rollback(() => {
              console.error("Error al insertar productos en el carrito:", error);
              res.status(500).json({ success: false, message: "Error al insertar productos en el carrito" });
            });
          });
      });
    });
