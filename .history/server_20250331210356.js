...
db.connect((err) => {
  ...
  app.post("/login", (req, res) => {
    ...
    console.log("Solicitud de login recibida:", { email, deviceId });
    
    // Primero verificamos las credenciales
    const sqlCheckUser = "SELECT * FROM freshboxusuarios WHERE us_email = ?";
    ...
    db.query(sqlCheckUser, [email], async (err, results) => {
      ...
      try {
        ...
        if (user.us_password.startsWith('$2')) { // Verificamos si es hash de bcrypt
          passwordMatch = await bcrypt.compare(password, user.us_password);
        } else {
          // Comparación directa (para contraseñas sin hash)
          passwordMatch = (user.us_password === password);
        }
      } catch (error) {
        console.error("Error al verificar contraseña:", error);
        // Si hay error en la verificación, intentamos comparación directa
        passwordMatch = (user.us_password === password);
      }
      ...
      if (user.us_sesion_activa && user.us_device_id !== deviceId) {
        return res.json({ 
          success: false, 
          code: "session_exists",
      ...
      const updateSession = `
        UPDATE freshboxusuarios 
        SET us_sesion_activa = TRUE, 
            us_device_id = ?, 
            us_last_activity = CURRENT_TIMESTAMP 
        WHERE us_id = ?
      `;
      
      db.query(updateSession, [deviceId, user.us_id], (updateErr) => {
        if (updateErr) {
  ...
  app.post("/heartbeat", (req, res) => {
    ...
    const updateActivity = `
      UPDATE freshboxusuarios 
      SET us_last_activity = CURRENT_TIMESTAMP 
      WHERE us_id = ? AND us_device_id = ?
    `;
    
    db.query(updateActivity, [userId, deviceId], (err) => {
      if (err) {
  ...
  app.post("/logout", (req, res) => {
    ...
    if (!userId) {
      return res.json({ success: false, message: "No se proporcionó ID de usuario" });
    }
    
    // Actualizar la base de datos
    const sqlLogout = `
      UPDATE freshboxusuarios 
      SET us_sesion_activa = FALSE, 
          us_device_id = NULL 
      WHERE us_id = ? 
      ${deviceId ? "AND us_device_id = ?" : ""}
    `;
    
    const params = deviceId ? [userId, deviceId] : [userId];
    
    db.query(sqlLogout, params, (err, result) => {
      if (err) {
        console.error("Error al cerrar sesión en la BD:", err);
        return res.status(500).json({ 
          success: false, 
  ...
  // Implementamos un limpiador de sesiones inactivas (opcional)
  const SESSION_TIMEOUT_MINUTES = 120; // 2 horas de inactividad
  ...
  function cleanInactiveSessions() {
    ...
    const query = `
      ...
      SET us_sesion_activa = FALSE, 
          us_device_id = NULL 
      WHERE us_sesion_activa = TRUE 
      AND us_last_activity < NOW() - INTERVAL ${timeoutMinutes} MINUTE
    `;
    
    db.query(query, (err, result) => {
      if (err) {
        console.error("Error al limpiar sesiones inactivas:", err);
      } else if (result.affectedRows > 0) {
        console.log(`Se limpiaron ${result.affectedRows} sesiones inactivas`);
      }
    });
  }
  
  // Ejecutar el limpiador cada 30 minutos
  setInterval(cleanInactiveSessions, 30 * 60 * 1000);
