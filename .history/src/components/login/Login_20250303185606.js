  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.data.message === "Inicio de sesión exitoso") {
        localStorage.setItem("userSession", response.data.user.us_email);
        localStorage.setItem("userType", response.data.user.us_tipo);
        setIsLoggedIn(true);
      ...
      if (response.data.us_tipo === "admin") {
        console.log("Email y Contraseña enviados:", { email, password });
        navigate("/admin-dashboard");  // Redirigir al admin dashboard
      } else {
        navigate("/homes");  // Redirigir al homes (cliente o transportista)
      }
    } else {
