...
db.connect((err) => {
  ...
  app.post("/login", async (req, res) => {
    ...
    if (req.session.user) {
      return res.json({ success: false, message: "Ya tienes una sesión activa" });
    }
    ...
    db.query(query, [email], async (err, results) => {
      ...
      req.session.user = { id: user.us_id, tipo: user.us_tipo };
  
      res.json({ success: true, us_id: user.us_id, us_tipo: user.us_tipo });
    });
  });
