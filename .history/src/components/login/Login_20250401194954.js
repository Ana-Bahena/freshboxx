...
function Login({ setIsLoggedIn }) {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            setErrors((prevErrors) => ({ 
                ...prevErrors, 
                login: response.data.message || "Usuario o contraseña incorrectos" 
            }));
        }
    } catch (error) {
  ...
  const handleRegister = (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      axios.post("http://localhost:5001/register", { nombre, email, password, telefono, direccion, rfc, numeroCuenta })
        .then((response) => {
          if (response.data.success) {
          ...
          <form onSubmit={handleRegister}>
            <div className="register-grid">
              ...
              <div className="form-group">
                ...
                <input type="text" placeholder="Número de Cuenta" value={numeroCuenta} onChange={(e) => setNumeroCuenta(e.target.value)} required />
              </div>
              <div className="register-column">
                <div className="form-group">
