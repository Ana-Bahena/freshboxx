...
const GraciasCompra = () => {
  ...
  const abrirModalFactura = async () => {
    ...
    try {
      const response = await fetch(`hhttp://localhost:5001/perfil?us_id=${userId}`);
  ...
  const descargarFactura = async () => {
    ...
    try {
      const response = await fetch(`https://fresh-back.onrender.com/descargar-factura?us_id=${userId}`, {
        method: "GET",
      });
