...
app.post("/actualizar-stock", (req, res) => {
  ...
  app.post('/api/finalizarcompra', (req, res) => {
    ...
    const { carrito, userId, vt_idFactura } = req.body;
    console.log('Cliente ID:', userId); 
  
    if (!carrito || carrito.length === 0 || !userId || !vt_idFactura) {
    ...
    db.beginTransaction((err) => {
      ...
      // Insertar en freshboxventas
      const ventaQuery = "INSERT INTO freshboxventas (vt_total, vt_fecha, vt_status, vt_pesoTotal, vt_formaPago, cl_id, vt_idFactura) VALUES (?, date, 'pendiente', ?, 'paypal', ?, ?)";
      db.query(ventaQuery, [total, fechaActual, 0, userId, vt_idFactura], (err, result) => {
        if (err) {
          return db.rollback(() => {
        ...
        // Insertar en freshboxcarritocompras
        const carritoQuery = "INSERT INTO freshboxcarritocompras (cc_cantidad, cc_subtotal, cc_precio, ts_id, vt_id, pr_id) VALUES ?";
...
src\components\carrito\Carrito.js
  Line 66:53:  'cl_id' is not defined  no-undef
  Line 71:12:  'cl_id' is not defined  no-undef
  Line 81:27:  'cl_id' is not defined  no-undef

Search for the keywords to learn more about each error., // Sin transportista asignado
          vt_id,   // Asociamos con la venta
          p.pr_id
        ]);
