...
app.post("/actualizar-stock", (req, res) => {
  ...
  app.post("/registrar-venta", (req, res) => {
    ...
    // Primero, registrar la venta en freshboxventas
    const queryVenta = `
      INSERT INTO freshboxventas (vt_total, vt_status, vt_pesoTotal, vt_formaPago, cl_id, vt_fecha)
      VALUES (?, 'pendiente', ?, 'paypal', ?, NOW())`;// Usamos NOW() para la fecha actual
  
    db.query(queryVenta, [vt_total, vt_pesoTotal, vt_formaPago, cl_id], (err, result) => {
      if (err) {
      ...
      // Ahora, registrar los productos del carrito en freshboxcarritocompras
      const queryCarrito = `
        INSERT INTO freshboxcarritocompras (cc_cantidad, cc_subtotal, cc_precio, ts_id, vt_id, pr_id)
        VALUES ?`;
  
      const valoresCarrito = carrito.map(p => [p.cantidad, p.cc_subtotal, p.cc_precio, null, vt_id, p.pr_id]);
  
      db.query(queryCarrito, [valoresCarrito], (err) => {
        if (err) {
          console.error("Error al registrar productos en el carrito:", err);
          return res.status(500).json({ success: false, message: "Error al registrar productos en el carrito" });
        }
