...
import { auth, googleProvider, signInWithRedirect, db, doc, getDoc, collection } from "../fire/FirebaseConfig";
...
function Login({ setIsLoggedIn }) {
  ...
  useEffect(() => {
    const checkUserAfterRedirect = async () => {
      try {
        ...
        if (result) {
          ...
          const response = await axios.post("http://localhost:5001/checkuser", { email });
          ...
          if (response.data.success) {
            ...
            console.log("Tipo de usuario recibido:", response.data.us_tipo);

            if (response.data.us_tipo === "cliente") {
              navigate("/homes");
            } else {
              navigate("/");
            }
          } else {
            alert("Este correo no está registrado en FreshBox.");
            await auth.signOut();
          }
        }
      } catch (error) {
