...
app.post("/alertas", (req, res) => {
  ...
  db.query(
    "INSERT INTO alertas (mensaje, tipo, estado) VALUES (?, ?, ?)",
    [mensaje, tipo, estado],
    (err, result) => {
      if (err) {
        return res.status(500).json({ success: false, message: "Error al agregar alerta" });
      }
      const alerta = { id: result.insertId, mensaje, tipo, estado };
...
app.put("/alertas/:id", (req, res) => {
  ...
  const { mensaje, tipo, estado } = req.body;
  db.query(
    "UPDATE alertas SET mensaje = ?, tipo = ?, estado = ? WHERE id = ?",
    [mensaje, tipo, estado, id],
    (err) => {
      if (err) {
        return res.status(500).json({ success: false, message: "Error al actualizar alerta" });
      }
      res.json({ success: true, message: "Alerta actualizada correctamente" });
    }
  );
});
...
app.delete("/alertas/:id", (req, res) => {
  const { id } = req.params;
  db.query("DELETE FROM alertas WHERE id = ?", [id], (err) => {
    if (err) {
