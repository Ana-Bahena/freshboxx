...
app.post("/actualizar-stock", (req, res) => {
  ...
  const buscarClienteQuery = "SELECT cl_cuenta, cl_id FROM freshboxclientes WHERE us_id = ?";
  db.query(buscarClienteQuery, [us_id], (err, results) => {
    if (err || results.length === 0) {
      console.error("Error al buscar cliente:", err || "Cliente no encontrado");
      return res.status(500).json({ success: false, message: "Cliente no encontrado para el usuario" });
    ...
    const cuentaCliente = cliente.cl_cuenta;  // Cuenta del cliente
    ...
    const cuentaDefault = "123456789";
          ...
          const transferirSaldoQuery = `
          START TRANSACTION;
          -- Verificar si las cuentas existen
          IF EXISTS (SELECT * FROM freshboxclientes WHERE cl_cuenta = ?) 
          AND EXISTS (SELECT * FROM freshboxclientes WHERE cl_cuenta = ?) THEN
            -- Obtener el saldo de la cuenta de origen
            SELECT cl_saldo INTO @saldo_actual_cuentaA
            FROM freshboxclientes WHERE cl_cuenta = ? FOR UPDATE;

            -- Verificar saldo suficiente
            IF @saldo_actual_cuentaA >= ? THEN
              -- Restar el monto de la cuenta cliente
              UPDATE freshboxclientes SET cl_saldo = cl_saldo - ? WHERE cl_cuenta = ?;
              -- Sumar el monto a la cuenta de destino (default)
              UPDATE freshboxclientes SET cl_saldo = cl_saldo + ? WHERE cl_cuenta = ?;
              
              -- Registrar la transacción
              INSERT INTO freshboxtransacciones (cl_id, tr_monto, tr_cuenta)
              VALUES (?, ?, ?);
          ...
          END IF;
        `;
        db.query(transferirSaldoQuery, [cuentaCliente, cuentaDefault, cuentaCliente, total, total, cuentaCliente, total, cuentaDefault, cliente.cl_id, total, cuentaDefault], (err) => {
          if (err) {
            return db.rollback(() => {
              console.error("Error en la transacción de saldo:", err);
...
const obtenerCliente = async (us_id) => {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT cl_nombre, cl_email, cl_telefono, cl_direccion, cl_rfc FROM freshboxclientes WHERE us_id = ?",
      [us_id]app.post('/api/finalizarcompra', (req, res) => {
        console.log('Solicitud recibida para finalizar compra:', req.body);
        const { carrito, us_id, vt_idFactura } = req.body;
            ...
            db.beginTransaction((err) => {
                ...
                db.query(ventaQuery, [total, fechaActual, 'pendiente', 0, 'paypal', us_id, vt_idFactura], (err, result) => {
                    ...
                    db.query(carritoQuery, [carritoValores], (err) => {
                        ...
                        // **Descontar saldo al cliente y transferirlo a la cuenta destino**
