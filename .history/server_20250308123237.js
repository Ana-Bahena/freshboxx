  ...
  app.post("/productos", (req, res) => {
    ...
    db.query(
      "INSERT INTO freshboxproductos (pr_nombre, pr_precio, pr_cantidad, pr_descripcion, pr_imagen, pr_temperatura, pr_status, pr_peso) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [pr_nombre, pr_precio, pr_cantidad, pr_descripcion, pr_imagen, pr_temperatura, pr_status, pr_peso],
      (err, result) => {
        if (err) {
          console.error("Error al agregar producto:", err);
          return res.status(500).json({ success: false, message: "Error al agregar producto" });
        }
        res.json({ success: true, message: "Producto agregado", producto: { pr_id: result.insertId, ...req.body } });
      }
    );
  });
