  ...
  function Carrito({ carrito, setCarrito }) {
    ...
    const handleLogin = async (e) => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          // Obtener el perfil del cliente y guardar el cl_id
          const perfilResponse = await axios.get("http://localhost:5001/perfil", { params: { us_id: response.data.us_id } });
          if (perfilResponse.data.success) {
            localStorage.setItem("clienteId", perfilResponse.data.cliente.cl_id);
          }
    ...
    const finalizarCompra = async () => {
      const cl_id = localStorage.getItem("clienteId"); // Recuperar el ID del cliente
