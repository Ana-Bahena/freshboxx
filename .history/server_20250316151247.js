...
app.post("/actualizar-stock", (req, res) => {
  ...
  app.post('/api/finalizar-compra', (req, res) => {
    ...
    const { carrito, cl_id } = req.body;

    if (!carrito || carrito.length === 0 || !cl_id) {
    ...
    db.beginTransaction((err) => {
      ...
      const ventaQuery = "INSERT INTO freshboxventas (vt_total, vt_fecha, vt_status, cl_id) VALUES (?, ?, 'pendiente', ?)";
      db.query(ventaQuery, [total, fechaActual, cl_id], (err, result) => {
        if (err) {
          return db.rollback(() => {
        ...
        const carritoValores = carrito.map(p => [
          ...
          p.precio * p.cantidad, 
          p.precio, 
          ts_id = null,  // ts_id asumiendo que corresponde a cl_id en este caso
        ...
        db.query(carritoQuery, [carritoValores], (err) => {
          ...
          const updateStockQueries = carrito.map(p => {
            return new Promise((resolve, reject) => {
              db.query(
