...
const filteredData = data.filter(incident => {
    ...
    // Convertir la fecha del incidente (DD/MM/YYYY → YYYY-MM-DD)
    const parts = incident.fecha.split("/"); // Divide la fecha "DD/MM/YYYY"
    if (parts.length !== 3) return false; // Si no tiene 3 partes, descartar

    const incidentDate = new Date(`${parts[2]}-${parts[1]}-${parts[0]}`); // Formato YYYY-MM-DD

    // Convertir fechas de entrada a objetos Date
    const start = new Date(startDate);
    const end = new Date(endDate);

    return incidentDate.getTime() >= start.getTime() && incidentDate.getTime() <= end.getTime();
});
