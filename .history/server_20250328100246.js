...
db.connect((err) => {
  ...
  const db = mysql.createConnection({
    ...
    password: process.env.DB_PASSWORD || "ana123",
...
app.get("/listacargas", (req, res) => {
  const query = `
    SELECT ct_id AS idContenedor, _id AS idVenta, ts_id AS idTransportista, ct_cantidad AS cantidadProductos 
...
app.get("/api/entregas", (req, res) => {
  const query = `
    SELECT 
      ...
      JOIN freshboxcontenedores c ON fc.vt_id = c.ct_id
