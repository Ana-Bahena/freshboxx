...
app.get("/descargar-factura", async (req, res) => {
  ...
  try {
    ...
    doc.text(`Cuenta: ${cliente.cl_rfc}`);
    doc.moveDown();
    ...
    const columns = [
      { title: "Producto", width: 160 },
      { title: "Cantidad", width: 80 },
      { title: "Precio sin IVA", width: 100 },
      { title: "IVA (16%)", width: 100 },
      { title: "Precio con IVA", width: 100 },
      { title: "Subtotal", width: 100 }
    ];
    ...
    columns.reduce((x, col) => {
      ...
      doc.text(col.title, x + 5, yPosition + 5, { width: col.width - 10, align: "center" });
      return x + col.width;
    }, startX);
    ...
    detallesVenta.forEach((producto) => {
      ...
      const iva = precioNum * 0.16;
      const precioConIVA = precioNum + iva;
      ...
      rowData.forEach((text, index) => {
        doc.rect(xPos, yPosition, columns[index].width, rowHeight).stroke();
        doc.text(text, xPos + 5, yPosition + 5, { width: columns[index].width - 10, align: "center" });
