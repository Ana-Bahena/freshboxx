...
export default function ControlCargas() {
  ...
  const obtenerContenedores = async () => {
    try {
      const response = await fetch("https://fresh-back.onrender.com/contenedores");
  ...
  const handleSubmitCargaContenedor = async (e) => {
    ...
    try {
      const response = await fetch("https://fresh-back.onrender.com1/agregar-carga-contenedor", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
