...
function Carrito({ carrito, setCarrito }) {
  ...
  useEffect(() => {
    ...
    if (!cl_id) {
      console.error("Error: clienteId no encontrado en localStorage.");
      alert("Por favor, verifica tu perfil antes de continuar.");
      navigate("/perfil");
    }
  }, [navigate]);
  ...
  // Verificar carrito para cantidades menores a 1
  useEffect(() => {
    const corregirCarrito = carrito.map(producto =>
      producto.cantidad < 1 ? { ...producto, cantidad: 1 } : producto
    );
    
    if (JSON.stringify(corregirCarrito) !== JSON.stringify(carrito)) {
      setCarrito(corregirCarrito);
    }
  }, [carrito, setCarrito]);
    ...
    <div className="carritoo-containeer">
      ...
      <p>Tu carrito está vacío.</p>
        ...
        <div className="productoos-container">
          ...
          >
            {!clienteId ? "Cargando información del cliente..." : "Finalizar Compra"}
          </button> 
        </div>
      )}
