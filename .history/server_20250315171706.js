...
app.post("/registrar-venta", (req, res) => {
  ...
  const queryVenta = `
    INSERT INTO freshboxventas (vt_total, vt_status, vt_pesoTotal, vt_formaPago, cl_id, pr_id)
    VALUES (?, 'pendiente', ?, ?, ?, ?)
  `;

  db.query(queryVenta, [vt_total, vt_pesoTotal, vt_formaPago, cl_id, carrito[0].pr_id], (err, result) => {
    if (err) {
    ...
    const queryCarrito = `
      INSERT INTO freshboxcarritocompras (cc_cantidad, cc_subtotal, cc_precio, ts_id, vt_id)
      VALUES ?
    `;

    const valoresCarrito = carrito.map(p => [p.cc_cantidad, p.cc_subtotal, p.cc_precio, null, vt_id]);
