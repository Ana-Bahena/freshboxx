  ...
  function Carrito({ carrito, setCarrito }) {
    ...
    const finalizarCompra = async () => {
      const cl_id = localStorage.getItem("cl_id");  // Leer el ID guardado
    
      if (!cl_id) {
        alert("No se encontró información del cliente. Inicia sesión nuevamente.");
        navigate("/login");  // O redirige a login
        return;
      }
      ...
      try {
        const response = await axios.post("http://localhost:3001/api/finalizarCompra", {
          carrito,
          cl_id: parseInt(cl_id)  // Asegúrate de enviarlo como número si es necesario
        });
